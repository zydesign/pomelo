 ...
 */
//pomelo.createApp()执行了app.init（）创建一个应用工厂函数，而app.init（）加载了默认配置appUtil.defaultConfiguration（）
module.exports.defaultConfiguration = function(app) {
  
  var args = parseArgs(process.argv); //进程参数
  setupEnv(app, args);                //设置环境：开发环境/产品环境
  loadMaster(app);                    //加载主机
  loadServers(app);                   //加载所有服务器
  processArgs(app, args);             //加载进程参数
  configLogger(app);                  //配置log
